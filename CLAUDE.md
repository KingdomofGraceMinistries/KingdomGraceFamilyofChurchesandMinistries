...
| 12 | **No hardcoded content — especially theology** - scripture, prayer prompts, devotionals, ministry challenges, Bible refs, or anything in the Bishop's voice must come from AI / DB / Bishop input — never from a JS constant. A blank slot is better than a fabricated verse. | `const FALLBACK = { scripture: "...", ref: "Proverbs 3:5" }`, inline default verses in template literals, "helpful" placeholder prayers in catch blocks |
