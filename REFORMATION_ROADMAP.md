...
**2. `SB_SECRET_KEY` holds the wrong key type.** The two-variable fallback is deliberate and stays: both keys remain configured until the legacy model is fully retired, so the functions keep working if the platform ever stops injecting the auto-provided variable. That design is correct and is not what needs changing.

What is wrong is the *value*. `SB_SECRET_KEY` currently holds a **publishable** key, which is anon-equivalent. Every function reads `SUPABASE_SERVICE_ROLE_KEY ?? SB_SECRET_KEY`, so if the first were ever empty the fallback would quietly hand all 12 functions **anon privileges** — audit writes rejected, service-role work blocked by RLS, logins failing in confusing ways. A fallback must preserve privilege, not reduce it: a backup key that cannot do the job is worse than a loud failure, because it half-works and the cause is hard to see.
